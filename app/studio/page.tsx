...
import { ModeRail, StudioHeader } from '@/app/components/TopBar'
...
import { buildSpineSkeleton, buildSpineAtlas } from '@/app/lib/spineExport'
...
export default function Home() {
  ...
  useEffect(() => {
    try {
      ...
      if (!k) {
        // No client key: only force BYOK if the server has no key either.
        // With a server-side OPENROUTER_API_KEY the studio runs on credits.
        fetch('/api/config')
          .then((r) => r.json())
          .then((cfg) => {
            if (!cfg?.serverKey) {
   ...
   */
  const ensureCanGenerate = async (usdOverride?: number): Promise<boolean> => {
    // Per-asset price in USD (paid in $PIXAI at the live rate, 100% burned).
    const PRICE_BY_MODE: Record<string, number> = {
      sprite: 2,
      tile: 3,
      parallax: 2.5,
      props: 2,
    }
    const usd = usdOverride ?? PRICE_BY_MODE[mode] ?? 2
  ...
  // Spine export: skeleton.json + .atlas + strip PNG, ready to import into Spine
  // (and any Spine runtime). Frames play as an attachment animation at the
  // sheet FPS; a body-plan bone skeleton ships at setup pose for binding art.
  const handleDownloadSpriteRig = async () => {
    try {
      const populated = spriteSheet.frames.filter((f) => !!f.imageUrl && !f.disabled)
      if (populated.length === 0) {
        setError(
          spriteSheet.frames.some((f) => !!f.imageUrl)
            ? 'All frames are excluded — include a frame before exporting the rig.'
            : 'Generate the sheet before exporting the rig.'
        )
        return
      }
      const cellUrls = populated.map((f) => f.imageUrl as string)
      ...
      const strip = await composeSpriteStripSheet(cellUrls)
      if (!strip) {
        setError('Could not compose the sprite strip for the rig export.')
        return
      }

      const skeleton = buildSpineSkeleton({
        bodyPlan: spriteBodyPlan,
        anim: spriteAnim,
        fps: spriteFps,
        frameCount: populated.length,
        frameSize: SPRITE_FRAME_SIZE,
        loop: spec.loop,
        imageFile,
      })
      const atlas = buildSpineAtlas({
        imageFile,
        frameCount: populated.length,
        frameSize: SPRITE_FRAME_SIZE,
      })
      ...
      const zip = new JSZip()
      const stripB64 = strip.split(',')[1]
      if (stripB64) zip.file(imageFile, stripB64, { base64: true })
      zip.file(`${baseName}.json`, JSON.stringify(skeleton, null, 2))
      zip.file(`${baseName}.atlas`, atlas)
      zip.file('manifest.json', JSON.stringify(buildSpriteManifest(populated), null, 2))
      zip.file(
        'README.txt',
        [
          'PIXAI — Spine rig export',
          '',
          `Body plan: ${BODY_PLANS[spriteBodyPlan].label}`,
          `Animation: ${spec.label} @ ${spriteFps} fps (${populated.length} frames)`,
          '',
          'Import into Spine: File → Import Data → ' + `${baseName}.json`,
          `Keep ${baseName}.json, ${baseName}.atlas and ${imageFile} together.`,
          '',
          'The frames play as an attachment animation. A body-plan bone skeleton',
          'is included at setup pose — attach your cut-up art to the bones to add',
          'true skeletal deformation.',
        ].join('\n')
      )
      ...
      const link = document.createElement('a')
      link.href = url
      link.download = `${baseName}_spine.zip`
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
      URL.revokeObjectURL(url)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to export rig')
    }
  }
    ...
    <div className="studio-light studio-shell">
      ...
      {isProps ? (
        ...
        <SpriteStudio
          ...
          onDownloadRig={handleDownloadSpriteRig}
        ...
        <section className="stage" style={{ gridColumn: '2 / -1' }}>
