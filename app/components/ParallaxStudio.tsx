  ...
  const viewport = (
    <div
      ...
      style={{
        ...
        background: 'linear-gradient(180deg, #f4f4f6 0%, #eaeaec 100%)',
      }}
    >
      {populated.length === 0 && (
        <div
                  ...
                  style={{
                    background: layer.imageUrl
                      ...
                      : isRecommended
                        ? 'var(--accent)'
                        : 'var(--surface)',
                    color: layer.imageUrl || isRecommended ? '#ffffff' : 'var(--text-muted)',
                    border: '1px solid var(--border)',
                  }}
                >
                  {layer.imageUrl ? '✓' : step}
...
export function ParallaxStudio({
  ...
  onGenerate,
  onDropFile,
  // Shell-provided primary Generate · pay in SOL button
  dock,
}: {
  layers: ParallaxLayer[]
  activeIdx: number
    ...
    <>
      <section className="stage">
        <div className="stage-toolbar">
          ...
          {!isEmpty && isResult && variantSelector}
          {!isEmpty && isResult && resultMessage && (
            <StatusPill status="ok" message={resultMessage} />
          )}
          {!isEmpty && !isResult && !loading && !autoExtending && (
            <span
              className="text-[12px]"
              style={{ color: 'var(--text-muted)' }}
            >
        ...
        <div className="stage-canvas">
          ...
          {isEmpty ? (
            ...
            <>
              <div className="stage-plate relative flex-1 anim-fade">
                {activeDirection && (
                  <div
      ...
      <aside className="inspector">
        <div className="inspector-header">
          <h2>Parallax scene</h2>
          <span>Layered scrolling background</span>
        </div>
        <div className="inspector-body">
          <div className="inspector-label">LAYERS</div>
          <LayerPanel
            layers={layers}
            activeIdx={activeIdx}
  ...
  return (
    <button
      ...
      onMouseEnter={(e) => {
        ...
        e.currentTarget.style.color = active ? '#ffffff' : 'var(--accent)'
      }}
      onMouseLeave={(e) => {
        e.currentTarget.style.borderColor = active
          ? 'var(--accent)'
          : 'var(--border-strong)'
        e.currentTarget.style.color = active ? '#ffffff' : 'var(--text-secondary)'
      }}
    >
      <Icon size={20} />
    </button>
